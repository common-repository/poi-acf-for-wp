...
abstract class POI_ACF_Location_Group extends POI_ACF_Singleton {
	...
	protected function __construct() {
		...
		$this->name = sprintf( __( 'Group %d', 'poi-acf-wp' ), self::$inc );
		$this->slug = 'group-' . ( self::$inc++ );

		// bad elliot.... bad
		// default to Forms group
		$this->primary_group_key = __( 'General', 'acf' );
	}
	...
	public function plugins_loaded() {
		...
		do_action( 'poi-acf-register-locations/' . $this->slug, $this );
	}
	...
	protected function _my_location_paths( $base_path ) {
		return apply_filters( 'poi-acf-location-paths/' . $this->slug, array( trailingslashit( $base_path ) . $this->slug . '/' ), $base_path, $this );
	}
	...
	protected function _load_locations() {
		...
		$base_dir = POI_ACF_WC_Launcher::instance()->plugin_dir() . 'includes/location/';
